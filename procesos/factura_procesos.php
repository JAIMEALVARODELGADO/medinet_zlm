...
switch($opcion) {
    ...
    case 'crearFacturaExterna':
        crearFacturaExterna($_GET['factura']);
        break;
    /*case 'crearRips':
...
function crearFacturaExterna($datos) {
    ...
    /*if(mysqli_query($conexion, $sql)) {
        return "Factura externa creada correctamente";
    } else {
        return "Error al crear la factura externa: " . mysqli_error($conexion);
    }*/

    //mysqli_query($conexion, $sql);
    $id_factura=mysqli_insert_id($conexion);
    //$id_factura = 100; // Simulación de ID de factura para pruebas
    ...
    foreach ($detalle as $item) {
        ...
        /*$id_servicio = mysqli_real_escape_string($conexion, $item['id_servicio']);
        $cantidad = mysqli_real_escape_string($conexion, $item['cantidad']);
        $valor_unitario = mysqli_real_escape_string($conexion, $item['valor_unitario']);
        $valor_total = mysqli_real_escape_string($conexion, $item['valor_total']);*/
        ...
        if(mysqli_num_rows($resultMed) > 0) {
            ...
            $sqlMedicamento = "INSERT INTO medicamento(nombre_mto, estado_mto, tipo_mto)
                            VALUES ('$descripcion', 'A', 'D')";
